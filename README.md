...
## Главная проблема алгоритма
При переходе к 3d координатам маркеров инвариантные от базиса величины нестабильны (видно по логам). Поэтому если сделать модель из более чем 1 контура, то с большой вероятностью они неправильно наложатся. Текущая настройка конфига создает модель из 1 контура, то есть просто призму из контура.   
